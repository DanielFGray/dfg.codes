...
import { EditorWithPreview } from '~/components/Editors'
...
export const meta = {
  ...
  date: '2017-04-17',
  description: "Introductions are hard, so I'm just gonna jump right into this.",
}
...
const User = (props) => (
  ...
  >
    <div style={{ float: 'right' }}>
      <img src={props.picture.large} style={{ borderRadius: '5px' }} alt="" />
    </div>
    <pre style={{ margin: 0 }}>{JSON.stringify(props, null, 2)}</pre>
  </div>
)

export default function UserList() {
  const [state, setState] = useState({ status: 'waiting', users: [] })
  function getUser() {
    setState((s) => ({ status: 'fetching', users: s.users }))
    fetch('https://randomuser.me/api')
      ...
      .then((res) =>
        setState((s) => ({
          users: res.results.concat(s.users),
          status: 'loaded',
        })),
      )
      .catch((error) =>
        setState((s) => ({ users: s.users, error, status: 'fail' })),
      )
  }
  ...
  return (
    <div>
      {state.error ? state.error.message || state.error : null}
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <button onClick={getUser}>add new user</button>
        {state.status}
      </div>
      <div>
        {state.users?.map((u) => (
          <User {...u} key={u.id.value} />
        ))}
      </div>
    </div>
  )
