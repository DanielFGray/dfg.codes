...
app.use((req, res) => {
  ...
  if (routerCtx.statusCode) res.status(routerCtx.statusCode)
...
One of the main things we're still missing is metadata in our `<head/>` tags. A few packages exist for this, the best I've found is [react-helmet-async](https://github.com/staylor/react-helmet-async).
