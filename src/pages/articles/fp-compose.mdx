...
```javascript
const yell = str => exclaim(upper(str))
```
...
```javascript
const yell = str => compose(exclaim, upper)(str)
```
...
Not only is it shorter, but I find it much easier to read, and we have much more re-usable and _composable_ code.
...
One immediate benefit of using composed functions can be seen when used with `Array.prototype.map`.
