...
export const meta = {
  ...
  category: 'computers',
  tags: ['sql', 'typescript', 'postgres', 'nodejs', 'react', 'supabase', 'programming'],
  date: '2022-06-02',
  description:
...
> Here, `slug` is used to identify what a comment belongs to.
  ...
  return (
    <li>
      ...
      {children.length > 0 && (
        <ul>
          {children.map(c => (
            <CommentCard key={c.comment_id} {...c} />
          ))}
        </ul>
      )}
    </li>
...
function useRealTimeComments(slug: string) {
...
function ReplyForm({ replyId, clearReplyId }: {
  replyId: null | number
  clearReplyId: () => void
}) {
...
function CommentCard({
  ...
  return (
    <li>
      ...
      {sessionUserId && (
        <div>
          <button onClick={() => setReplyId(comment.comment_id)}>reply</button>
