...
The type of functions used in _reducers_ is `<A, B>(a: A, b: B) => A`, that is, a function that takes two arguments and returns a value that's the same type as the first argument.
`reduce` itself has the signature `<A, B>(fn: (acc: B, val: A) => B, init: B, arr: A[]) => B`: it accepts a reducer function, a thing `B`, a list of `A`s, and returns a thing the same type as `B`.
...
`map` has a type signature of `<A, B>(fn: (a: A) => B, arr: A[]) => B[]`, it's a function that receives a function from `A` to `B`, a list of `A`s, and returns a list of `B`s.
...
<EditorWithConsole
  file={`
const flatMap = (fn, a) => a.reduce((p, c) => p.concat(fn(c)), [])
...
A slight twist on filter, this splits an array into two arrays based on whether they match a predicate function:
...
That example above, taking an array of objects and retrieving the value of a particular property from each one, is a common enough pattern that I'd like to make a special function for it:
