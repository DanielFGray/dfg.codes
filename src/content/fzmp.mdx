...
date: '2014-11-02'
description: 'I recently came across fzf, which is an interactive filter for your shell, and wanted to make a script that filtered through and played my music library'
---
...
After talking to the folks at `freenode.net/#bash` they pointed out I would be better off using an array for multiple files, and `mapfile` is perfect for this. The biggest benefit to using an array here is that I have built-in functionality for counting the elements, rather than repeatedly calling `wc -l`.

```bash
mapfile -t songs < <(mpc listall | fzf -m)
```
