  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <button
        onClick={onStop}
        disabled={isLoading}
        className="rounded-full bg-gray-300 p-2 text-gray-700 transition-colors hover:bg-gray-400 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-gray-600 dark:text-gray-200 dark:hover:bg-gray-500"
        title="Stop"
      >
        <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 20 20">
      ...
      <button
        ...
        disabled={isLoading}
        className={`rounded-full p-2 transition-colors ${metronomeVolume > 0
            ? 'bg-blue-500 text-white'
            : 'bg-gray-300 text-gray-700 dark:bg-gray-600 dark:text-gray-200'
          } hover:bg-blue-400 disabled:cursor-not-allowed disabled:opacity-50`}
        title={metronomeVolume > 0 ? 'Metronome On' : 'Metronome Off'}
      >
        <svg
          className={`h-5 w-5 ${metronomeVolume > 0 ? 'text-white' : ''}`}
          fill="currentColor"
          viewBox="0 0 314 314"
        >
